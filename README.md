...
3. Position the robot with the servo keys, then press **C** to choose a reference pose after a four-second countdown. The servos stay powered and hold their current commanded positions throughout. Calibration captures the next complete detected pose after the countdown and makes each current servo position its displayed zero without changing its target. C preserves your run/pause choice. Use the keys to position the robot: these servos provide no shaft-position feedback for hand positioning. A servo key cancels calibration and immediately returns to keyboard control.
...
GPIO6 left-hand rotation, GPIO7 elbow, and GPIO8 wrist sensitivity are **4x**: a 5-degree change from your reference requests a 20-degree servo change before optional smoothing. Claw gain is **100x**, ten times its previous 10x response. Keyboard steps are 7 servo degrees. Rotating the whole right forearm without bending the wrist does not change the wrist's relative angle.
...
Hand sensitivity is set separately with `hand_confidence = 0.2` in `config.toml`; pose confidence is `0.4`. Lower thresholds make detection more permissive and can also admit less reliable detections. The settings control MediaPipe's detection, presence, and tracking thresholds; see the [official Hand Landmarker options](https://developers.google.com/edge/mediapipe/solutions/vision/hand_landmarker/python#configuration_options). Left/right ownership comes from arm positions, with clear wrist matches accepted within 18% of the image diagonal; ambiguous overlaps remain unassigned. A clipped or invalid unused fingertip no longer drops a whole hand. Wrist, rotation, and pinch each validate only their own required points, allowing a 5% image-edge margin while keeping the hand wrist in frame. A missing pinch tip can therefore leave wrist movement available.

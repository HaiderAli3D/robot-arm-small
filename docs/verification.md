...
## Follow-up: more permissive hand detection

Reduced default and supplied hand confidence from 0.35 to 0.2 and pose confidence from 0.6 to 0.4. Relaxed clear wrist-association distance from 12% to 18% of the frame diagonal while retaining ambiguity rejection. An unused clipped/invalid fingertip no longer rejects a whole hand: pinch, wrist bend, and rotation independently validate the landmarks they need. Preview rendering now skips invalid hand points and edges to avoid crashes when partially usable hands are retained.

The final combined changes pass all 194 Python tests, including independent control availability, ambiguous wrist rejection, lower pose confidence, wrist estimate offsets, and rendering partially invalid hands. A real camera-1 dry run processed 120 frames and exited cleanly with mean inference 23.56 ms. It recorded zero frames containing all four controls; this smoke test establishes runtime operation, not a measured improvement in live hand detection accuracy.

...
class KeyRepeaterTests(unittest.TestCase):
    def setUp(self):
        self.keys = KeyRepeater(delay=.5, repeat_hz=60)
    ...
    def test_hold_repeats_at_sixty_hz_after_initial_delay(self):
        self.keys.event(0x62, 1, 0)
        self.assertEqual(self.keys.poll(0, {0x62}), {ord('2'): 1})
        self.assertEqual(self.keys.poll(.24, {0x62}), {})
        self.assertEqual(self.keys.poll(.49, {0x62}), {})
        self.assertEqual(self.keys.poll(.5, {0x62}), {ord('2'): 1})
        self.assertEqual(self.keys.poll(.6, {0x62}), {ord('2'): 6})
    ...
    def test_native_repeat_does_not_add_steps(self):
        ...
        for t in (.2, .3, .4):
            ...
            self.assertEqual(self.keys.poll(t, {0x62}), {})
    ...
    def test_camera_stall_does_not_replay_seconds_of_old_repeats(self):
        ...
        self.assertEqual(self.keys.poll(5.017, {0x62}), {ord('2'): 1})
